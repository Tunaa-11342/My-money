...
import { HandCoins, Wallet, PiggyBank, Receipt, Coins } from "lucide-react";
...
export function CashflowSummaryCards({
  ...
  const totalBudget = q.data?.currentMonthTotalBudget ?? 0;
  const carryOver = q.data?.currentMonthCarryOver ?? 0;
  ...
  const items = [
    ...
    {
      title: `Tổng quỹ khả dụng${monthKey ? ` (${monthKey})` : ""}`,
      value: totalBudget,
      icon: (
        <Coins className="h-10 w-10 text-sky-400 bg-sky-400/10 rounded-xl p-2" />
      ),
...
return (
  <div className="relative w-full grid grid-cols-1 md:grid-cols-6 gap-6 py-4">
    {items.map((item, i) => {
      const isTopRow = i < 3;
      ...
      return (
        <div
          ...
          className={cn(
            "md:col-span-3",          
            isTopRow && "md:col-span-2" 
          )}
        >
          <SkeletonWrapper isLoading={q.isFetching}>
            <div
              className={cn(
